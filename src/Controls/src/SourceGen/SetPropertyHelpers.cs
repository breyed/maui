...
static class SetPropertyHelpers
{
	public static void SetPropertyValue(IndentedTextWriter writer, ILocalValue parentVar, XmlName propertyName, INode valueNode, SourceGenContext context, NodeSGExtensions.GetNodeValueDelegate? getNodeValue = null, bool treeOrder = false, IndentedTextWriter? icWriter = null, ILocalValue? inflatorVar = null, bool asCollectionItem = false)
	{
		...
		// Heuristic: If BP is null but we're trying to set a BindingBase and the type has a property/field
		// with a BindablePropertyAttribute, assume the BP will be generated by another source generator
		if (!asCollectionItem && bpFieldSymbol == null && IsBindingBaseNode(valueNode, context) &&
			parentVar.Type.HasBindablePropertyHeuristic(localName, context))
		{
			SetBindingWithHeuristic(writer, parentVar, localName, valueNode, context, getNodeValue);
			return;
		}
	...
	static void SetBindingWithHeuristic(IndentedTextWriter writer, ILocalValue parentVar, string localName, INode node, SourceGenContext context, NodeSGExtensions.GetNodeValueDelegate getNodeValue)
	{
		var localVariable = getNodeValue((ElementNode)node, context.Compilation.ObjectType);
		// Generate SetBinding call using the expected BindableProperty name that should be generated by another source generator
		var bpName = $"{localName}Property";
